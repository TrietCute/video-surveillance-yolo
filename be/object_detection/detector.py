...
class Detector:
    def __init__(self, cam_id: str, event_queue: Queue = None):
        ...
        self.frame_queue = Queue(maxsize=300)  # buffer ghi video
    ...
    def detect_on_frame(self, frame):
        ...
        person_boxes, weapon_boxes, animal_boxes = [], [], []
        for r in results:
            for box in r.boxes:
        ...
        fall_detected = False
        if person_boxes:
            fall_detected = analyze_pose(frame, source_id=self.cam_id, video_path="")
        ...
        abnormal = bool(animal_boxes or (person_boxes and self.outside_working_hours()) or
                        (person_boxes and weapon_boxes) or fall_detected)
        ...
        if abnormal:
            ...
            if not self.abnormal_state:
                ...
                if self.event_queue:
                    self.event_queue.put({"type": "start", "frame": frame.copy(), "annotated": annotated, "timestamp": now})
            else:
                if self.event_queue:
